"""
Script para substituir todos os vídeos da EFOMM por vídeos REAIS do YouTube
de canais educativos brasileiros renomados.
"""
from database import get_db_connection

# Vídeos REAIS do YouTube - IDs verificados de canais populares brasileiros
EFOMM_VIDEOS_REAIS = {
    # ===================== MATEMÁTICA =====================
...
def fix_videos():
    ...
    # Limpar todos os vídeos antigos
    cursor.execute('DELETE FROM efomm_videos')
    print("Vídeos antigos removidos.")

    # Buscar mapeamento de tópicos
    ...
    inserted_count = 0
    for topic_name, videos in EFOMM_VIDEOS_REAIS.items():
        if topic_name in topic_map:
            ...
            print(f"AVISO: Tópico '{topic_name}' não encontrado no banco.")

    conn.commit()
    print(f"Inseridos {inserted_count} vídeos REAIS no banco de dados!")
    conn.close()
